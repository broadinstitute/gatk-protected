#
# Case sample workflow for a single pair of case-control samples. Includes GATK CNV and ACNV. Supports both WGS and WES samples.
# EXPERIMENTAL
#############

workflow case_gatk_acnv_single_pair_workflow {
    ...
    call BayesianHetPulldownPaired {
      input:
          entity_id_tumor=case_id,
          entity_id_normal=control_id,
          gatk_jar=gatk_jar,
          ref_fasta=ref_fasta,
          ref_fasta_fai=ref_fasta_fai,
          ref_fasta_dict=ref_fasta_dict,
          tumor_bam=case_bam,
          tumor_bam_idx=case_bai,
          normal_bam=control_bam,
          normal_bam_idx=control_bai,
    ...
    call PlotSegmentedCopyRatio {
      input:
          ...
          called_file=TumorCaller.called_file,
          output_dir="${plots_dir}CopyRatio_Plots/${case_id}/",
          mem=4
    }
    ...
    call PlotACNVResults {
      input:
          ...
          tn_file=TumorNormalizeSomaticReadCounts.tn_file,
          output_dir="${plots_dir}ACNV_plots/${case_id}",
          mem=4
    }
    ...
    call CNLoHAndSplitsCaller {
      input:
          ...
          acnv_segments=AllelicCNV.acnv_final_segs,
          tn_file=TumorNormalizeSomaticReadCounts.tn_file,
          output_dir="${call_cnloh_dir}CallCNLoH/${case_id}",
          mem=4
    }
