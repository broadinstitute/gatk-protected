...
/**
 * Unit tests for {@link PloidyAnnotatedTargetCollection}.
 *
 * @author Mehrtash Babadi &lt;mehrtash@broadinstitute.org&gt;
 */
public class PloidyAnnotatedTargetCollectionUnitTest extends BaseTest {
    ...
    private static List<ContigPloidyAnnotation> contigPloidyAnnotsList;
    private static List<Target> targetList;
    private static PloidyAnnotatedTargetCollection ploidyAnnotsCollection;

    @BeforeClass
    public static void loadTestResources() {
        try {
            contigPloidyAnnotsList = ContigPloidyAnnotationTableReader.readContigPloidyAnnotationsFromFile(CONTIG_PLOIDY_ANNOTS_FILE);
        ...
        ploidyAnnotsCollection = new PloidyAnnotatedTargetCollection(contigPloidyAnnotsList, targetList);
    }
    ...
    public void testTargetPloidy() {
        ...
        Assert.assertEquals(ploidyAnnotsCollection.getTargetPloidyByTag(testAutosomalTarget, "SEX_XX"), 2);
        Assert.assertEquals(ploidyAnnotsCollection.getTargetPloidyByTag(testAutosomalTarget, "SEX_XX"), 2);

        /* on X contig */
        final Target testXTarget = new Target("UNNAMED_TARGET_182568");
        Assert.assertEquals(ploidyAnnotsCollection.getTargetPloidyByTag(testXTarget, "SEX_XX"), 2);
        Assert.assertEquals(ploidyAnnotsCollection.getTargetPloidyByTag(testXTarget, "SEX_XY"), 1);

        /* on Y contig */
        final Target testYTarget = new Target("UNNAMED_TARGET_189476");
        Assert.assertEquals(ploidyAnnotsCollection.getTargetPloidyByTag(testYTarget, "SEX_XX"), 0);
        Assert.assertEquals(ploidyAnnotsCollection.getTargetPloidyByTag(testYTarget, "SEX_XY"), 1);
    }

    @Test(expectedExceptions = IllegalArgumentException.class)
    public void testNonexistentPloidyTag() {
        ploidyAnnotsCollection.getTargetPloidyByTag(new Target("UNNAMED_TARGET_0"), "FOO");
    }

    @Test(expectedExceptions = IllegalArgumentException.class)
    public void testNonexistentTarget() {
        ploidyAnnotsCollection.getTargetPloidyByTag(new Target("UNNAMED_TARGET_FOO"), "SEX_XX");
    }
    ...
    public void testNonUniqueTargetNames() {
        ...
        new PloidyAnnotatedTargetCollection(contigPloidyAnnotsList, badTargetList);
    }

    /* check if ploidy map is empty */
    @Test(expectedExceptions = UserException.BadInput.class)
    public void testEmptyPloidyMap() {
        final List<ContigPloidyAnnotation> badContigAnnotsList = Arrays.stream(new ContigPloidyAnnotation[] {
                new ContigPloidyAnnotation("1", ContigClass.AUTOSOMAL, new HashMap<>())
        }).collect(Collectors.toList());
        new PloidyAnnotatedTargetCollection(badContigAnnotsList, targetList);
    }
    ...
    public void testMultiplyAnnotatedContigs() {
        ...
        final List<ContigPloidyAnnotation> badContigAnnotsList = Arrays.stream(new ContigPloidyAnnotation[] {
                new ContigPloidyAnnotation("1", ContigClass.AUTOSOMAL, ploidyMap),
                new ContigPloidyAnnotation("1", ContigClass.ALLOSOMAL, ploidyMap)
        }).collect(Collectors.toList());
        new PloidyAnnotatedTargetCollection(badContigAnnotsList, targetList);
    }
