...
public class AllelicCountReader extends TableReader<AllelicCount> {
    ...
    static AllelicCount createAllelicCountWithVerbosity(final DataLine dataLine, final AllelicCountTableColumn.AllelicCountTableVerbosity verbosity) {
        ...
        if (verbosity == AllelicCountTableColumn.AllelicCountTableVerbosity.BASIC) {
            ...
            final Nucleotide refNucleotide = Nucleotide.valueOf(dataLine.get(AllelicCountTableColumn.REF_NUCLEOTIDE).getBytes()[0]);
            final Nucleotide altNucleotide = Nucleotide.valueOf(dataLine.get(AllelicCountTableColumn.ALT_NUCLEOTIDE).getBytes()[0]);
            final int readDepth = dataLine.getInt(AllelicCountTableColumn.READ_DEPTH);
