...
public final class AllelicCountWithPhasePosteriors extends AllelicCount {
    ...
    public AllelicCountWithPhasePosteriors(final AllelicCount count,
        ...
        if (!Double.isFinite(refMinorProb) || !Double.isFinite(altMinorProb) || !Double.isFinite(outlierProb)) {
            throw new IllegalArgumentException("Cannot construct AllelicCountWithPhasePosteriors with non-finite probabilities at: " + count.getInterval());
        }
    }
