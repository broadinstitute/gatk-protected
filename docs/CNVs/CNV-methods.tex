...
Let us define $\lambda_{st} \equiv \PP_{st} c_{st} e^{b_{st}}$ as the total multiplicative bias per sample per target. Assuming that $\lambda_{st}$ is sharply peaked around its mode, the MLE for $d_s$ can be easily found by approximating the Poisson distributions with Gaussian like before:
\begin{align}
...
\end{equation}
Maximizing yields a quadratic equation with one acceptable solution:
\begin{equation}
d_s = \frac{\sqrt{4\llangle \vn^2 \rrangle_s \llangle \boldsymbol{\lambda}^2 \rrangle_s + \llangle \boldsymbol{\lambda} \rrangle_s^2} - \llangle \boldsymbol{\lambda} \rrangle_s}{2 \llangle \boldsymbol{\lambda}^2 \rrangle_s},
\end{equation}
where we have defined the masked target average $\llangle \cdot \rrangle_s$ as:
\begin{equation}
\llangle \mathbf{v} \rrangle_s \equiv \frac{\sum_t M_{st} v_{st}}{\sum_t M_{st}}.
\end{equation}
If $n_{st} \gg 1$, we may approximate the result as $d_s \simeq \left(\llangle \vn^2 \rrangle_s / \llangle \boldsymbol{\lambda}^2 \rrangle_s\right)^{\frac{1}{2}}$. Before learning the parameters of the coverage model, we may set $\lambda_{st} \rightarrow \PP_{st}$ to obtain a first estimate of $d_s$. This estimate can be updated along the way as better estimates for $c_{st}$ and $b_{st}$ is found.
...
\right.
\end{equation}
where $\rho$ is {\em average read count per base per strand}, $l_t$ is the target length (number of overlapping bases), and $\epss_M \sim 10^{-5}$ is the (small) probability of a mapping error that may result in a (small) number of reads to be mapped to the Y chromosome for an XX genotype sample. Similarly, for the YY genotype we have:
\begin{equation}
...
\noindent {\bf Compound reads likelihood:} we define compounds reads on X and Y chromosomes as $N_X \def \sum_{t \in T_X} n_t$ and $N_Y \def \sum_{t \in T_Y} n_t$, respectively. Both of these compound random variables have a Poisson distribution with the sum of the Poisson parameter of the underlying reads. The compounds likelihoods are given as:
\begin{align}
...
\end{equation}
If the compounds reads likelihood model is used, the compound likelihood functions must be accordingly:
\begin{equation}
