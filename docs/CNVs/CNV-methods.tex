...
Let us define $\lambda_{st} \equiv \PP_{st} c_{st} e^{b_{st}}$ as the total multiplicative bias per sample per target. Assuming $\lambda_{st}$ is sharply peaked about its mode, the MLE for $d_s$ can be easily found by approximating the Poisson distributions with a Gaussian distribution as discussed earlier: 
\begin{align}
